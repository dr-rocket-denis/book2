...
def main():
    ...
    pg.mixer.init()
    intro_text = [
        ' Орбитальный аппарат Марса столкнулся с ошибкой при выведении на орбиту.',
        ' Используйте двигатели для коррекции на круговую картографическую орбиту без',
        ' заканчивается топливо или сгорает в атмосфере.'
        ]
    instruct_text1 = [
        'Высота орбиты должна быть в пределах 69-120 миль',
        'Эксцентриситет орбиты должен быть < 0,05',
        'Избегайте верхней части атмосферы на высоте 68 миль'    
        ]
    instruct_text2 = [
        'Стрелка Влево = Уменьшить Dx', 
        'Стрелка Вправо = Увеличить Dx', 
        'Стрелка Вверх = Уменьшение Dy', 
        'Стрелка вниз = Увеличение Dy', 
        'Пробел = Очистить путь',
        'Побег = Выход На Весь экран'        
        ]  
    planet = Planet()
    ...
    while running:
        ...
        if eccentricity < 0.05 and sat.distance >= 69 and sat.distance <= 120:
            map_instruct = ['Нажмите и удерживайте М для картографирования влажности грунта']
        ...
        if pg.time.get_ticks() <= 15000: 
            instruct_label(screen, intro_text, GREEN, 75, 100)
        ...
        instruct_label(screen, instruct_text2, WHITE, 500, 510)
        cast_shadow(screen)
