...
withdrawal_1 = default_input("Ввод ежегодного вывода средств до уплаты налогов за первые 5 лет (в сегодняшних долларах): \n", '100000')
while not withdrawal_1.isdigit():
    withdrawal_1 = input("Неверный ввод! Только входное целое число: ")
withdrawal_2 = default_input("Введите ежегодное снятие средств до уплаты налогов на остаток (в сегодняшних долларах): \n", '80000')
while not withdrawal_2.isdigit():
    withdrawal_2 = input("Неверный ввод! Только входное целое число: ")
...
def bankrupt_prob(outcome, bankrupt_count):
    ...
    print("Ежегодный вывод средств первые 5 лет: ${:,}".format(int(withdrawal_1)))
    print("Ежегодный вывод средств после 5 лет: ${:,}".format(int(withdrawal_2)))
